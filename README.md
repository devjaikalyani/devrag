...
DevRAG has two tiers. You bring your own LLM API keys on both — DevRAG never marks up model usage.

| | Free | Pro |
|---|---|---|
| Price | Rs 0 / $0 forever | Rs 199/month or $3.99/month (Rs 1,999 / $39.99 yearly — 2 months free) |
| Indexed repositories | 2 | Unlimited |
| Chat queries | 25/day | Unlimited |
| Agent runs | 5/month (local tasks, dry runs, no-PR) | Unlimited, including GitHub issue to pull request |
| Model routing | Standard | Architect-model routing (Opus-tier) for the hardest tasks |
| Extras | — | Run history export, cost analytics, priority support, early access |

The anchor: one automated PR fix saves more developer time than a Pro month costs.

### Payments (Razorpay)

Payments run on Razorpay Orders — one integration for both markets:

- Domestic (India): INR via UPI, cards, netbanking, and wallets
- International: USD via cards (enable International Payments in your Razorpay dashboard)

Setup:

1. Create API keys at the Razorpay dashboard and set `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET` in `.env`.
2. Optional (hosted deployments): add a webhook for `payment.captured` pointing at `https://your-host/billing/webhook` and set `RAZORPAY_WEBHOOK_SECRET`.
3. The Upgrade button on `/pricing` creates an order, opens Razorpay Checkout, verifies the payment signature server-side, and activates Pro locally.

Purchases are one-time per period (no auto-renewal); buying again extends the current expiry. With no Razorpay keys configured, everything simply runs on the free tier.

| Route | Purpose |
|---|---|
| `GET /billing/plans` | Pricing catalog |
| `GET /billing/status` | Current tier, expiry, remaining free quota |
| `POST /billing/order` | Create a Razorpay order (`{package, currency}`) |
| `POST /billing/verify` | Verify checkout signature, activate Pro |
| `POST /billing/webhook` | Razorpay server-to-server confirmation |

Free-tier limit hits return HTTP 402 with an upgrade hint.

Legal pages live at `/legal/terms`, `/legal/privacy`, `/legal/refunds`, and `/legal/contact`
(Razorpay account verification requires these on the public site).

## Responsible use

- Run PR mode only against repositories you own or have contributor consent for.
  Unsolicited automated pull requests violate GitHub's Acceptable Use Policies.
  DevRAG enforces this: PR-mode runs on repos your token does not own are refused
  unless you explicitly set `ALLOW_THIRD_PARTY_REPOS=true`.
- Review every agent-generated pull request before merging. DevRAG proposes changes;
  a human ships them. Every PR is labeled "Generated by DevRAG".
- The agent executes test commands from cloned repositories. Until sandboxed
  execution ships (roadmap item 1), only run it against repositories you trust.
...
1. Sandboxed test execution — Docker-per-run isolation. Required before accepting
   untrusted repositories; promoted to the top after a security review.
2. GitHub App mode — comment `@devrag fix` on any issue to trigger a run; webhook triage. This is the distribution channel that matters most.
3. MCP server — expose ingest/search/solve as MCP tools so Claude Code, Cursor, and any MCP client can use DevRAG as their code-understanding backend.
4. Incremental indexing — file-hash watch mode so indexes stay warm on every commit instead of full re-ingestion.
5. Tree-sitter chunking — true AST-aware chunk boundaries for all supported languages, replacing regex heuristics.
...
DevRAG is source-available under the Elastic License 2.0 (see [LICENSE](LICENSE)).
You may use, modify, and self-host it freely. You may not offer DevRAG to third
parties as a hosted or managed service, or circumvent the license and entitlement
functionality. All runtime dependencies are permissively licensed (MIT/BSD/Apache-2.0).
